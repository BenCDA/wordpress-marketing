 ...
 * @return array
 */
function digitalmarketingagency_body_classes( $classes ) {
...
add_filter( 'body_class', 'digitalmarketingagency_body_classes' );
...
if (!function_exists('digitalmarketingagency_str_replace_assoc')) {

    /**
     * digitalmarketingagency_str_replace_assoc
     * @param  array $replace
     * @param  array $subject
     * @return array
     */
    function digitalmarketingagency_str_replace_assoc(array $replace, $subject) {
 ...
 */
function digitalmarketingagency_add_to_cart_fragment( $fragments ) {
	...
	<a class="nav-link cart-icon-wrap header-cart" href="javascript:void(0);" role="button" data-bs-toggle="collapse" data-bs-target="#Cart">
		...
		?>	
			<span><?php echo esc_html_e('0','digital-marketing-agency'); ?></span>
		<?php
		}
		?>
	</a>
...
add_filter( 'woocommerce_add_to_cart_fragments', 'digitalmarketingagency_add_to_cart_fragment' );
...
if ( ! function_exists( 'digitalmarketingagency_breadcrumb_title' ) ) {
	function digitalmarketingagency_breadcrumb_title() {
		...
		elseif ( is_day() ) : 
			...
			printf( __( 'Daily Archives: %s', 'digital-marketing-agency' ), get_the_date() );
		
		elseif ( is_month() ) :
		
			printf( __( 'Monthly Archives: %s', 'digital-marketing-agency' ), (get_the_date( 'F Y' ) ));
			
		elseif ( is_year() ) :
		
			printf( __( 'Yearly Archives: %s', 'digital-marketing-agency' ), (get_the_date( 'Y' ) ) );
			
		elseif ( is_category() ) :
		
			printf( __( 'Category Archives: %s', 'digital-marketing-agency' ), (single_cat_title( '', false ) ));

		elseif ( is_tag() ) :
		
			printf( __( 'Tag Archives: %s', 'digital-marketing-agency' ), (single_tag_title( '', false ) ));
			
		elseif ( is_404() ) :

			printf( __( 'Error 404', 'digital-marketing-agency' ));
			
		elseif ( is_author() ) :
		
			printf( __( 'Author: %s', 'digital-marketing-agency' ), (get_the_author( '', false ) ));		
...
function digitalmarketingagency_breadcrumbs() {
	
	$showOnHome	= esc_html__('1','digital-marketing-agency'); 	// 1 - Show breadcrumbs on the homepage, 0 - don't show
	$delimiter 	= '';   // Delimiter between breadcrumb
	$home 		= esc_html__('Home','digital-marketing-agency'); 	// Text for the 'Home' link
	$showCurrent= esc_html__('1','digital-marketing-agency'); // Current post/page title in breadcrumb in use 1, Use 0 for don't show
    ...
    if ( is_category() ) 
	{
		...
		echo $before . esc_html__('Archive by category','digital-marketing-agency').' "' . esc_html(single_cat_title('', false)) . '"' .$after;
		
	} 
	
	elseif ( is_search() ) 
	{
		echo $before . esc_html__('Search results for','digital-marketing-agency').' "' . esc_html(get_search_query()) . '"' . $after;
	} 
	...
	elseif ( is_tag() ) 
	{
		echo $before . esc_html__('Posts tagged ','digital-marketing-agency').' "' . esc_html(single_tag_title('', false)) . '"' . $after;
	} 
	
	elseif ( is_author() ) {
		global $author;
		$userdata = get_userdata($author);
		echo $before . esc_html__('Articles posted by ','digital-marketing-agency').'' . $userdata->display_name . $after;
	} 
	
	elseif ( is_404() ) {
		echo $before . esc_html__('Error 404 ','digital-marketing-agency'). $after;
    }
	
    if ( get_query_var('paged') ) {
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
		echo ' ( ' . esc_html__('Page','digital-marketing-agency') . '' . esc_html(get_query_var('paged')). ' )';
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
    }
...
add_action( 'wp_ajax_digitalmarketingagency_dismissed_notice_handler', 'digitalmarketingagency_ajax_notice_handler' );

/**
 * AJAX handler to store the state of dismissible notices.
 */
function digitalmarketingagency_ajax_notice_handler() {
    if ( isset( $_POST['type'] ) ) {
...
add_action( 'wp_ajax_install_act_plugin', 'digitalmarketingagency_admin_install_plugin' );

function digitalmarketingagency_admin_install_plugin() {
    /**
     * Install Plugin.
     */
