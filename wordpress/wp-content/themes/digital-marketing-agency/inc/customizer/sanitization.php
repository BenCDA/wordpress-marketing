 ...
 */
function digitalmarketingagency_sanitize_checkbox( $checked ) {
	// Boolean check.
	return ( ( isset( $checked ) && true == $checked ) ? true : false );
}


function digitalmarketingagency_sanitize_float( $input ) {
    return filter_var($input, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
}


function digitalmarketingagency_switch_sanitization( $input ) {
    if ( true === $input ) {
        return 1;
    } else {
        return 0;
...
function digitalmarketingagency_sanitize_choices( $input, $setting ) {
    global $wp_customize; 
 ...
 */
function digitalmarketingagency_sanitize_hex_color( $hex_color, $setting ) {
 ...
 */
function digitalmarketingagency_sanitize_html( $html ) {
 ...
 */
function digitalmarketingagency_sanitize_image( $image, $setting ) {
 ...
 */
function digitalmarketingagency_sanitize_nohtml( $nohtml ) {
 ...
 */
function digitalmarketingagency_sanitize_number_absint( $number, $setting ) {
 ...
 *                    the setting default.
 */
function digitalmarketingagency_sanitize_range_value( $number, $setting ) {
 ...
 */
function digitalmarketingagency_sanitize_select( $input, $setting ) {
 ...
 */
function digitalmarketingagency_sanitize_url( $url ) {
	return esc_url_raw( $url );
}


/* Sanitization Text*/
function digitalmarketingagency_sanitize_text( $text ) {
	return wp_filter_post_kses( $text );
}

/* Sanitization Integer*/
function digitalmarketingagency_sanitize_integer( $input ) {
    if( is_numeric( $input ) ) {
        return intval( $input );
    }
}
